...
exports.reconcileWalletBalances = void 0;
...
exports.reconcileWalletBalances = (0, scheduler_1.onSchedule)({
    ...
    v2_1.logger.info("[reconcileWalletBalances] Starting reconciliation run.");
    // ── Iterate all users ─────────────────────────────────────────────────
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        for (const walletDoc of walletsSnap.docs) {
            ...
            // ── Correct the balance ─────────────────────────────────────────
            v2_1.logger.warn(`[reconcileWalletBalances] Drift detected uid=${uid} wallet=${walletId} ` +
                `stored=${currentBalance} true=${trueBalance} drift=${drift}`);
            await walletDoc.ref.update({
                balance: trueBalance,
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
                lastReconciled: firestore_1.FieldValue.serverTimestamp(),
    ...
    v2_1.logger.info(`[reconcileWalletBalances] Done. Users=${totalUsersProcessed} ` +
        `WalletsFixed=${totalWalletsFixed}`);
});
//# sourceMappingURL=reconcileBalances.js.map