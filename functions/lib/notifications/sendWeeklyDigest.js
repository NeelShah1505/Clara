...
exports.sendWeeklyDigest = (0, scheduler_1.onSchedule)({
    ...
    const lastMonday = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() - daysToLastMonday));
    ...
    v2_1.logger.info(`[sendWeeklyDigest] Sending digest for week ${from} → ${to}`);
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        for (const doc of txSnap.docs) {
            ...
            if (d.type === "expense") {
                ...
                const cid = d.categoryId || "__other__";
                categoryTotals.set(cid, (categoryTotals.get(cid) ?? 0) + d.amount);
            }
            if (d.type === "income")
        ...
        const topLine = topCategoryName
            ...
            : "";
        const body = `${txSnap.size} transactions. Spent: ${fmt(totalExpense)}.${netLine}${topLine}`;
        // Send the notification
        const tokenDocs = await db.collection(`users/${uid}/fcmTokens`).get();
        const tokensList = tokenDocs.docs.map((d) => d.data().token).filter(Boolean);
        ...
        try {
            ...
            v2_1.logger.error(`[sendWeeklyDigest] FCM failed for uid=${uid}:`, err);
        }
    }
    v2_1.logger.info(`[sendWeeklyDigest] Done. Notifications sent=${sent}`);
});
// ── Helpers ───────────────────────────────────────────────────────────────────
